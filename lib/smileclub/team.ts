...
const TASKS_RAW: TeamTask[] = [
  ...
  { key: 'g-announce', who: 'gautam', wk: 1, dueIso: '2026-09-28', due: 'Mon 28 Sep', task: 'Partnership announcement — Dr. Tosun Dental Clinic is now part of Dental Nation',
    objective: 'Agree the facts, the date and the order of the announcement with Dr. Yahya Tosun, so patients hear the news from their own dentist before any Smile Club video or message from the clinic.',
    why: 'Every Dr. Tosun Dental Clinic script says “part of Dental Nation”, but patients do not know yet. Announced well, it reads as an upgrade; discovered in a Smile Club message, it reads as a surprise.',
    steps: [
      { s: 'Facts confirmed', how: 'Team, clinic, prices, patient records and accepted insurance stay the same — or say exactly what changes. The script promises “nothing you value changes”.' },
      { s: 'Date agreed', how: 'With Dr. Yahya Tosun, from the roadmap you started together; tell MJ so the shoot is booked before it.' },
      { s: 'Script approved', how: 'Dr. Tosun, Ms Shadi and Dr Luvi approve the announcement script (Dentist scripts → Dr. Yahya Tosun → Video 3).' },
      { s: 'Announcement out', how: 'Video and a WhatsApp message to the clinic’s patients go out before the clinic’s Smile Club waves start.' },
    ],
    done: 'Patients told; the clinic’s Smile Club campaign can start.', with: 'Dr. Yahya Tosun · Fahad · MJ', to: 'scripts', toLabel: 'Dentist scripts', weight: 2, subs: 0, subsNote: 'Enabler — unlocks the Dr. Tosun Dental Clinic share of the dentists’ 24' },
  ...
  { key: 'l-labcoats', who: 'luvi', wk: 1, dueIso: '2026-09-28', due: 'Mon 28 Sep', task: 'DN-branded lab coats — received and handed out before the shoots',
    objective: 'Procurement has ordered DN-branded lab coats for the dentists who have fit or availability issues with the current DN scrubs. Receive them on Mon 28 Sep and get each coat to its dentist before their shoot.',
    why: 'Every dentist appears on camera in Dental Nation branding that fits well. Without the coats, those dentists’ shoots slip.',
    steps: [
      { s: 'Dentists named', how: 'List which dentists need a lab coat and tell Mohan, so anyone filming on Sat 26 or Sun 27 Sep without well-fitting scrubs moves to their backup day from Mon 28 Sep.' },
      { s: 'Delivered', how: 'Procurement’s delivery arrives (expected Mon 28 Sep); check sizes against the list.' },
      { s: 'Handed out', how: 'Each coat to its dentist at their branch before their shoot slot.' },
    ],
    done: 'Every dentist filming from Mon 28 Sep has a DN lab coat that fits.', with: 'Procurement · Mohan', to: 'scripts', toLabel: 'Shoot schedule', weight: 1, subs: 0, subsNote: 'Enabler — keeps the shoots on schedule' },
  ...
  { key: 'm-shoot-tosun', needs: ['g-announce'], who: 'mohan', wk: 1, dueIso: '2026-09-28', due: 'Mon 28 Sep, 10:00 pencilled — new date after Dr. Tosun’s Türkiye trip', task: 'Shoot 1 — Dr. Yahya Tosun (Dr. Tosun Dental Clinic)', scripts: ['yahya-tosun'],
    ...
    steps: [
      { s: 'Scripts approved', how: 'Dr. Yahya Tosun reads all three scripts below in both languages and approves or adjusts them; the Turkish is checked for natural wording. The announcement facts are confirmed by Gautam first.' },
      { s: 'Consent ready', how: 'Written consent forms for anyone else who appears on camera. Bring the prompter.' },
      { s: 'Filmed — six takes', how: 'Announcement in Turkish, then English; change the framing; Smile Club in Turkish, then English; DN Scan in Turkish, then English. Clinical areas tidy, no patient identifiable without consent.' },
...
const SHOOT_TASKS: TeamTask[] = SHOOT_PLAN.filter((day) => !day.tasks && day.stops.some((st) => st.slots.some((x) => !x.task))).map((day) => {
  ...
  return {
    ...
    steps: [
      ...
      { s: 'Filmed', how: 'Two videos per dentist, first language then English, change the framing between videos. The dentist wears well-fitting DN scrubs or the DN-branded lab coat (arriving Mon 28 Sep). Written consent from anyone else on camera.' },
